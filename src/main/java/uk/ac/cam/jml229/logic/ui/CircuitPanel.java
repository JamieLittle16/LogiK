...
public class CircuitPanel extends JPanel {
  ...
  private class InputHandler extends MouseAdapter {
    ...
    public void mousePressed(MouseEvent e) {
      ...
      Component clickedComp = getLogicComponentAt(e.getPoint()); // Fixed method name
    ...
    public void mouseMoved(MouseEvent e) {
      Component c = getLogicComponentAt(e.getPoint());
      setCursor(c != null ? Cursor.getPredefinedCursor(Cursor.HAND_CURSOR) : Cursor.getDefaultCursor());
    }
  }
  ...
  private void drawComponents(Graphics2D g2) {
    for (Component c : components) {
      ...
      // Draw Shape (Now passing isSelected to handle specific highlights)
      if (c instanceof Switch)
  ...
  // --- Component Drawers (With Selection Logic) ---

  private void drawSwitch(Graphics2D g2, Switch s, int x, int y, boolean isSelected) {
    // 1. Draw Selection Halo (Circle/Rounded)
    if (isSelected) {
      g2.setColor(SELECTION_BORDER);
      g2.setStroke(new BasicStroke(5)); // Thick glow
  ...
  private void drawLight(Graphics2D g2, OutputProbe p, int x, int y, boolean isSelected) {
    // 1. Selection Halo (Circle)
    if (isSelected) {
      g2.setColor(SELECTION_BORDER);
  ...
  private void drawGenericBox(Graphics2D g2, Component c, int x, int y, boolean isSelected) {
    if (isSelected) {
      g2.setColor(SELECTION_BORDER);
      g2.setStroke(new BasicStroke(5));
  ...
  private void drawAndGate(Graphics2D g2, Component c, int x, int y, boolean isSelected) {
    ...
    fillAndOutlineGate(g2, path, isSelected);
    drawInputPins(g2, x, y, 2);
  }

  private void drawOrGate(Graphics2D g2, Component c, int x, int y, boolean isSelected) {
    Path2D path = new Path2D.Double();
    path.moveTo(x, y);
  ...
  private void drawXorGate(Graphics2D g2, Component c, int x, int y, boolean isSelected) {
    // Draw back line
    ...
    // Draw the rest as an OR gate
    drawOrGate(g2, c, x + 5, y, isSelected);
  }

  private void drawNotGate(Graphics2D g2, Component c, int x, int y, boolean isSelected) {
    Path2D path = new Path2D.Double();
    path.moveTo(x, y);
  ...
  private void fillAndOutlineGate(Graphics2D g2, Path2D path, boolean isSelected) {
    // 1. Draw Highlight (Halo) BEHIND the black outline
    if (isSelected) {
      g2.setColor(SELECTION_BORDER);
      g2.setStroke(new BasicStroke(5)); // Thicker stroke for visibility
      g2.draw(path);
    }
